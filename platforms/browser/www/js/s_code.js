...
/************************ ADDITIONAL FEATURES ************************
 Plugins
 */
if (!s_account){ var s_account = "aokde"; }
var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.cookieLifetime=2*365*86400 // Cookie lifetime in seconds
s.currencyCode="EUR"
s.charSet="UTF-8"
...
s.linkInternalFilters="javascript:,aok.de"
...
function s_doPlugins(s) {
    ...
    if (s.pageType=="errorPage") s.pageName="404:<"+document.location.href+">";
    /* s.setupFormAnalysis(); */
    s.prop8=s.getTimeParting("h","+1");        // Hour of the day
    s.eVar1=s.prop8;
    s.prop9=s.getTimeParting("d","+1");        // Day of the week
    s.eVar2=s.prop9;
    s.prop10=s.getTimeParting("w","+1");        // Weekday/Weekend
    s.eVar3=s.prop10;
    s.prop11=s.getDaysSinceLastVisit();
    s.eVar4=s.prop11;
    s.campaign=s.getQueryParam('cid');
    s_campaign=s.getAndPersistValue(s.campaign,'sc_perscmp',0);
    s.campaign=s.getValOnce(s.campaign,'s_campaignid',7);
    if (s_campaign) {
        s.prop13=s.pageName + "|" + s_campaign;
        s.eVar5=s.pageName + "|" + s_campaign;
    }
    s.eVar20=s.getQueryParam('icd');
    s.eVar20=s.getValOnce(s.eVar20,"s_icd",0); //de-dup campaign responses
}
...
s.getDaysSinceLastVisit=new Function(""
    ...
    +"60*60*1000;e.setTime(ct+s.cookieLifetime*1000);cval=s.c_r(c);if(!cval){s.c_w(c,"
